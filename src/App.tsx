...
import { GolfCourse, RoundRecord } from './types';
import { SAMPLE_COURSES } from './data/courses';
import { useSettings } from './hooks/useSettings';
import { useRoundHistory } from './hooks/useRoundHistory';
import { useLearningData } from './hooks/useLearningData';
import { useOneShotLocation } from './hooks/useGeolocationWatch';
...
import { Play, MapPin, Trophy, ChevronRight, Settings, Sparkles, Zap, ShieldCheck } from 'lucide-react';

const AVG_CLUB_REPLACEMENT_COST = 150;

export default function App() {
  const { settings, updateSettings } = useSettings();
  const { history, addRound, clearHistory } = useRoundHistory();
  const { learningData, recordConfirmation, recordCompletedRound, resetLearning } = useLearningData();
  const { lat: userLat, lng: userLng } = useOneShotLocation();
  ...
  const handleCompleteOnboarding = (updated: Partial<typeof settings>) => {
    updateSettings({ ...updated, onboardingCompleted: true });
  };
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-stone-100 flex flex-col font-sans antialiased selection:bg-emerald-500 selection:text-emerald-950">
      <HeaderNavigation activeTab={activeTab} setActiveTab={setActiveTab} isRoundActive={isRoundActive} gpsActive={true} />

      <main className="flex-1 w-full max-w-md mx-auto">
        {!settings.onboardingCompleted && <OnboardingModal settings={settings} onComplete={handleCompleteOnboarding} />}
        ...
        {activeTab === 'home' && (
          <div className="p-4 space-y-5 pb-20">
            ...
            <div className="bg-gradient-to-br from-emerald-900 via-emerald-950 to-stone-900 border border-emerald-800/80 rounded-3xl p-5 shadow-2xl relative overflow-hidden space-y-4">
              ...
              <div>
                <h1 className="text-3xl font-black text-white tracking-tight">Never leave a club behind.</h1>
                <p className="text-xs text-emerald-200/90 mt-1.5 leading-relaxed">
                  Automatic green departure detection announces <strong>"Club Check."</strong> before you drive away. No
                  sensors or RFID tags needed.
                </p>
              </div>
              ...
              >
                <div className="space-y-0.5">
                  <div className="text-[10px] uppercase font-bold text-emerald-400 tracking-wider">Selected Course</div>
                  <div className="text-sm font-bold text-white group-hover:text-emerald-300 transition-colors">{selectedCourse.name}</div>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              >
                ...
                <div className="text-[10px] text-emerald-400">~${totalClubsSaved * AVG_CLUB_REPLACEMENT_COST} saved cost</div>
              </div>
              ...
              >
                ...
                <div className="text-[10px] text-stone-400">Voice: "{settings.voicePhrase}"</div>
              </div>
            </div>
            ...
            <div className="bg-stone-900 border border-stone-800 rounded-2xl p-4 space-y-3">
              ...
              <div className="space-y-2 text-xs text-stone-400">
                <div className="flex items-start space-x-2">
                  ...
                  <span>
                    <strong>Zero Hardware:</strong> Works entirely using your phone's built-in motion & GPS.
                  </span>
                </div>
                <div className="flex items-start space-x-2">
                  <span className="text-emerald-400 font-bold">•</span>
                  <span>
                    <strong>Instant Simulator:</strong> Includes built-in green departure simulator for indoor testing.
                  </span>
                </div>
                <div className="flex items-start space-x-2">
                  <span className="text-emerald-400 font-bold">•</span>
                  <span>
                    <strong>No Accounts:</strong> 100% private. All round history remains strictly on device.
                  </span>
                </div>
              </div>
            </div>
        ...
        {activeTab === 'active_round' && (
          <ActiveRound course={selectedCourse} settings={settings} onEndRound={handleEndRound} onRecordConfirmation={recordConfirmation} />
        )}

        {activeTab === 'history' && <RoundHistory history={history} learningData={learningData} onClearHistory={handleClearHistory} />}

        {activeTab === 'settings' && (
          <SettingsView settings={settings} onUpdateSettings={updateSettings} learningData={learningData} onResetLearning={handleResetLearning} />
        )}
