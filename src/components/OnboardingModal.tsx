...
const PLAYING_STYLES: { id: PlayingStyle; label: string; desc: string; icon: string }[] = [
...
const TARGET_PACES: { id: TargetPace; label: string; sub: string }[] = [
...
export const OnboardingModal: React.FC<OnboardingModalProps> = ({ settings, onComplete }) => {
  ...
  const requestGps = () => {
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        () => setGpsGranted(true),
        () => setGpsGranted(true) // Still allow the user to proceed with the simulator
      );
    } else {
      setGpsGranted(true);
    }
  ...
  const handleFinish = () => {
    onComplete({ onboardingCompleted: true, playingStyle, targetPace });
  };
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <p className="text-emerald-200/90 text-sm leading-relaxed">
                ClubCheck learns when you leave a green and delivers a simple, timely reminder before you drive or walk
                away.
              </p>
              ...
              <div className="bg-emerald-900/40 rounded-xl p-4 border border-emerald-800/80 space-y-3">
                <div className="flex items-start space-x-3 text-xs text-emerald-200">
                  ...
                  <span>
                    <strong>Zero Hardware:</strong> No club sensors, no RFID tags, no expensive accessories.
                  </span>
                </div>
                <div className="flex items-start space-x-3 text-xs text-emerald-200">
                  <span className="text-emerald-400 font-bold">•</span>
                  <span>
                    <strong>Zero Friction:</strong> Start your round once, put your phone away, and listen.
                  </span>
                </div>
                <div className="flex items-start space-x-3 text-xs text-emerald-200">
                  <span className="text-emerald-400 font-bold">•</span>
                  <span>
                    <strong>100% Private:</strong> GPS location and round data remain strictly on your device.
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 gap-3">
                {PLAYING_STYLES.map((item) => (
                  <button
                    key={item.id}
                    onClick={() => setPlayingStyle(item.id)}
          ...
          {step === 3 && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 gap-2.5">
                {TARGET_PACES.map((item) => (
                  <button
                    key={item.id}
                    onClick={() => setTargetPace(item.id)}
              ...
              <div className="space-y-3">
                <div className="bg-emerald-900/40 p-3.5 rounded-xl border border-emerald-800 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition ${
                      gpsGranted ? 'bg-emerald-500 text-emerald-950' : 'bg-emerald-700 hover:bg-emerald-600 text-white'
                    }`}
                  >
                ...
                <div className="bg-emerald-900/40 p-3.5 rounded-xl border border-emerald-800 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition ${
                      audioTested ? 'bg-emerald-500 text-emerald-950' : 'bg-emerald-700 hover:bg-emerald-600 text-white'
                    }`}
                  >
