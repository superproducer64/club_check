...
import { evaluateGreenDeparture, msToMph } from '../utils/geo';
import { recordConfirmationFeedback } from '../utils/storage';
...
export const ActiveRound: React.FC<ActiveRoundProps> = ({
  ...
  // Simulation controls
  const [simMode, setSimMode] = useState<string>('idle'); // 'idle' | 'approaching' | 'putting' | 'departing'
  ...
  useEffect(() => {
    ...
    } else if (departureState === 'APPROACHING') {
    ...
    if ('geolocation' in navigator) {
      const watchId = navigator.geolocation.watchPosition(
        (pos) => {
          ...
          // Evaluate departure math against current green coordinates
          ...
          if (evalResult.shouldTriggerReminder) {
            triggerReminderAlert(currentHole.holeNumber, evalResult.distanceToGreenMeters);
          }
        },
        (err) => {
  ...
  }, [isLiveGps, currentHole, departureState, dwellTimeSeconds, settings]);

  const triggerReminderAlert = useCallback((holeNum: number, distMeters: number) => {
    setActiveReminderHole(holeNum);
  }, []);

  const handleConfirmReminder = (retrievedClub: boolean) => {
    // Record feedback in local storage to update totalConfirmations in learning data
    recordConfirmationFeedback(true, retrievedClub);
  ...
  // --- Simulation Helper Actions for Indoors / Testing ---
  ...
  const simStepDriveAwayDeparture = () => {
    ...
    // Delay 2 seconds then trigger voice reminder
    setTimeout(() => {
      triggerReminderAlert(currentHole.holeNumber, 32);
    ...
    <div className="max-w-md mx-auto p-4 space-y-4 text-stone-100 pb-20">
      ...
      <div className="bg-stone-900 border border-stone-800 rounded-2xl p-3 flex items-center justify-between">
        <button
          ...
          onClick={() => {
            setCurrentHoleIdx((i) => Math.max(0, i - 1));
            setDepartureState('APPROACHING');
        ...
        <button
          ...
          onClick={() => {
            setCurrentHoleIdx((i) => Math.min(course.holes.length - 1, i + 1));
            setDepartureState('APPROACHING');
            setDistanceToGreen(150);
          }}
          className="p-2 rounded-xl bg-stone-950 border border-stone-800 text-stone-300 disabled:opacity-40 hover:bg-stone-800"
      ...
      <div className="bg-stone-900 border border-stone-800 rounded-2xl p-4 space-y-3">
        <h4 className="text-xs font-bold text-stone-300 uppercase tracking-wider">
          Round Reminders Log ({reminderEvents.length})
        </h4>

        {reminderEvents.length === 0 ? (
          <div className="text-xs text-stone-500 italic py-2">
            No green departures recorded yet for this round.
          </div>
        ) : (
          <div className="space-y-2">
            {reminderEvents.map((evt) => (
              <div
                key={evt.id}
                className="bg-stone-950 p-2.5 rounded-xl border border-stone-800/80 flex items-center justify-between text-xs"
              >
      ...
      {activeReminderHole !== null && (
        <ClubCheckReminderModal
          holeNumber={activeReminderHole}
          settings={settings}
