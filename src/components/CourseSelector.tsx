...
const NEARBY_COURSES_API_KEY = 'B5Z4DM2AMTA7HULMXDHF3DEZSI';

export const CourseSelector: React.FC<CourseSelectorProps> = ({ selectedCourse, onSelectCourse, onClose, userLat, userLng }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [showCustomModal, setShowCustomModal] = useState(false);
  const [customName, setCustomName] = useState('');
  const [customCity, setCustomCity] = useState('');

  const [apiCourses, setApiCourses] = useState<GolfCourse[]>([]);
  ...
  const fetchNearbyApiCourses = async (lat: number, lng: number) => {
    ...
    try {
      const res = await fetch(`/api/courses/nearby?lat=${lat}&lng=${lng}`, {
        headers: { Authorization: `Bearer ${NEARBY_COURSES_API_KEY}` },
      });
      if (res.ok) {
  ...
  const handleManualApiSearch = () => {
    fetchNearbyApiCourses(userLat ?? 36.568, userLng ?? -121.947);
  };
  ...
  const filteredCourses = allCourses.filter(
    (c) => c.name.toLowerCase().includes(searchTerm.toLowerCase()) || c.location.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleCreateCustom = (e: React.FormEvent) => {
    ...
    handleSelectAndClose(newCourse);
    setShowCustomModal(false);
  };

  const handleNearestCourse = () => {
    const nearest = apiCourses[0] ?? SAMPLE_COURSES[0];
    if (nearest) handleSelectAndClose(nearest);
  };
    ...
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-stone-900 text-stone-100 rounded-2xl border border-stone-800 shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="p-4 bg-stone-950 border-b border-stone-800 flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-stone-400 hover:text-white text-xs font-semibold px-2.5 py-1 rounded-lg bg-stone-800">
            Close
          </button>
        </div>
        ...
        <div className="p-4 bg-stone-900/90 border-b border-stone-800 space-y-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            >
              {isLoadingApi ? <Loader2 className="w-3.5 h-3.5 animate-spin text-emerald-400" /> : <RefreshCw className="w-3.5 h-3.5 text-emerald-400" />}
              ...
              return (
                <button
                  ...
                  className={`w-full p-3.5 rounded-xl text-left border transition-all flex items-start justify-between ${
                    isSelected ? 'bg-emerald-950 border-emerald-500 text-white shadow-md' : 'bg-stone-950/70 border-stone-800/80 hover:bg-stone-800/60 text-stone-200'
                  }`}
                >
                  <div className="space-y-1">
                    <div className="font-bold text-sm text-white flex items-center space-x-2">
                      ...
                      {isSelected && (
                        <span className="bg-emerald-500 text-emerald-950 text-[10px] font-bold px-2 py-0.5 rounded-full">Active</span>
                      )}
                      {course.isLiveFetched && (
                        <span className="bg-blue-950 text-blue-400 border border-blue-800 text-[10px] font-mono px-1.5 py-0.5 rounded">Live API</span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center space-x-3 text-xs text-stone-400">
                      ...
                      {course.distanceKm !== undefined && (
                        <span className="text-emerald-400 font-mono text-[11px]">({course.distanceKm} km away)</span>
                      )}
                      <span className="flex items-center space-x-1">
                        <Flag className="w-3 h-3 text-emerald-500" />
                        <span>
                          Par {course.parTotal} ({course.totalYards} yds)
                        </span>
                      </span>
                    </div>
                  </div>
        ...
        {showCustomModal && (
          <div className="p-4 bg-stone-950 border-t border-stone-800 space-y-3">
            <h4 className="text-xs font-bold text-emerald-400 uppercase tracking-wider">Create Unlisted Golf Course</h4>
            <form onSubmit={handleCreateCustom} className="space-y-3">
              ...
              <div className="flex justify-end space-x-2">
                ...
                <button type="submit" className="px-4 py-1.5 rounded-lg text-xs font-bold bg-emerald-500 text-emerald-950 hover:bg-emerald-400">
                  Save & Select
                </button>
              </div>
            </form>
