...
import { Flag, Navigation, ShieldCheck, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export const GreenRadarMap: React.FC<GreenRadarMapProps> = ({
  ...
  // Calculate visual dot position (center is green at 120, 120)
  const mapCenter = 120;
  const scale = 80 / maxVisualRangeMeters; // 80px visual radius for max range
  
  // Offsets based on departure state
  let cartOffsetY = 0;
  if (departureState === 'APPROACHING') {
    cartOffsetY = clampedDistance * scale;
  } else if (departureState === 'ON_GREEN') {
    cartOffsetY = Math.min(15, clampedDistance * scale);
  } else if (departureState === 'DEPARTING' || departureState === 'CLEAR') {
    cartOffsetY = clampedDistance * scale;
  } else {
    cartOffsetY = clampedDistance * scale;
  }
  ...
  const getStateBadge = () => {
    switch (departureState) {
      ...
      case 'ON_GREEN':
        return {
          label: `On Green (${dwellTimeSeconds}s)`,
    ...
    <div className="bg-stone-900 border border-stone-800 rounded-2xl p-4 space-y-3 relative overflow-hidden shadow-inner">
      ...
      <div className="flex items-center justify-between text-xs">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-stone-400">Par {par} • {yardage}y</span>
        </div>
      ...
      <div className="relative w-full aspect-square max-w-[260px] mx-auto bg-stone-950 rounded-full border border-stone-800 shadow-2xl flex items-center justify-center p-2">
        <svg viewBox="0 0 240 240" className="w-full h-full overflow-visible">
          ...
          <g transform={`translate(120, ${120 + cartOffsetY})`} className="transition-transform duration-500 ease-out">
            ...
            <line x1="0" y1="0" x2="0" y2={departureState === 'DEPARTING' ? '12' : '-12'} stroke="#38bdf8" strokeWidth="2" strokeLinecap="round" />
          </g>
        </svg>
