...
/**
 * Plays a pleasant double-chime audio tone using the Web Audio API
 */
...
export interface SpeakAlertOptions {
  volume?: number;
  pitch?: number;
  rate?: number;
  tone?: AudioTone;
...
/**
 * Announces speech using the Web Speech Synthesis API, honoring the app's audio tone setting
 */
export function speakAlert(text: VoicePhrase | string, options: SpeakAlertOptions = {}): Promise<boolean> {
  const { volume = 1.0, pitch = 1.0, rate = 1.0, tone = 'standard', haptic = true } = options;

  if (haptic) {
    triggerHapticVibration();
  }

  if (tone === 'silent' || tone === 'vibrate_only') {
