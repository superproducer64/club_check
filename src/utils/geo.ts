 ...
 * - Inner green area: <= 20m triggers ON_GREEN
...
export function evaluateGreenDeparture(
  ...
  const currentSpeedMph =
    currentCoord.speed !== null && currentCoord.speed >= 0 ? msToMph(currentCoord.speed) : 0;
  ...
  switch (currentState) {
    case 'APPROACHING':
      ...
      break;
    case 'ON_GREEN':
      // Minimum dwell time on green before a departure counts (avoids false triggers)
